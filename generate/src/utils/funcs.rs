...
pub fn parse_sdjwt_paylod(
    ...
    match serialization_format {
        ...
        SDJWTSerializationFormat::GeneralJson => parse_payload_general_json(sd_jwt, remove_decoy),
        SDJWTSerializationFormat::Compact => parse_payload_compact(sd_jwt, remove_decoy),
    }
}
...
fn parse_payload_general_json(sd_jwt: &str, remove_decoy: bool) -> Result<Value> {
    ...
    let signature = parsed.signatures.first().ok_or_else(|| {
        Error::from_msg(
            ErrorKind::Input,
